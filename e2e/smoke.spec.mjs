...
const collectRuntimeErrors = (page) => {
  ...
  page.on('console', (message) => {
    ...
    // The normal web build imports Google Fonts; CI may block that network request.
    if (/fonts\.googleapis\.com|fonts\.gstatic\.com|Failed to load resource/i.test(text)) return;
...
test('production web app: TSV generation and SVG/JPEG export work', async ({ page }) => {
...
test('portable single-file build opens directly and can generate a figure', async ({ browser }) => {
